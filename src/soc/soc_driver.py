...
#        H0_b = S_AO @ C_b @ np.diag(eps_b) @ C_b.T @ S_AO   # utils.ao_fock_from_mos(S_AO, C_b, eps_b)
    ...
    if not config.UKS:
        ...
        pop_lowdin_mat_sf = utils.ao_population_lowdin_with_ops(mo_coeffs_mat, np.zeros_like(mo_coeffs_mat), lowdin_ops)
        ...
        pop_lowdin_mat_sf_a = utils.ao_population_lowdin_with_ops(mo_coeffs_mat_a, np.zeros_like(mo_coeffs_mat_a), lowdin_ops)
        ...
        pop_lowdin_mat_sf_b = utils.ao_population_lowdin_with_ops(mo_coeffs_mat_b, np.zeros_like(mo_coeffs_mat_b), lowdin_ops)
    ...
    if config.UKS:
        ...
        C_a_ortho = utils.chol_orthonormalize(C_a, S_AO)
        C_b_ortho = utils.chol_orthonormalize(C_b, S_AO)
        ...
        # Pre-multiply S once (already optimal)
        S_alpha = S_AO @ alpha
        S_beta  = S_AO @ beta
    
        proj_a = C_a_ortho.conj().T @ S_alpha
        proj_b = C_b_ortho.conj().T @ S_beta
        proj   = np.vstack([proj_a, proj_b])
    
        w_sf = (proj.conj() * proj).real
        w_sf /= np.clip(w_sf.sum(axis=0, keepdims=True), 1e-16, None)
    
        proj_abs2_for_conn   = w_sf
        E_sf_all_Ha_for_conn = np.concatenate([eps_a, eps_b])
        ...
        proj_a = C_ortho.conj().T @ S_alpha
        proj_b = C_ortho.conj().T @ S_beta
